...
public interface IUsuarioDao extends CrudRepository<Usuario, Integer> {
    ...
    @Query(value = "SELECT COUNT(u.id) FROM Usuario u WHERE u.usuario = ?1", nativeQuery = true)
    Integer existsByUsuario(String usuario);

    @Query(value = "SELECT COUNT(u.id) FROM Usuario u WHERE u.email = ?1", nativeQuery = true)
